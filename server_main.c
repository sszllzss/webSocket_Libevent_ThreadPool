 ...
 * # > Revise Time: 2018-10-09 15:08:23
 *  ************************************************************************/
    ...
    { 
        ...
        { 
        	...
        	{		
				...
				accept_fd = accept(ws->fd, (struct sockaddr *)&acceptAddr, (socklen_t *)&socAddrLen);  
...
{
    int ret;
    ret = webSocket_recv(fd ,(unsigned char *) buf , bufLen);    // 使用websocket recv
    if(ret > 0)
	{
		...
		if(strstr(buf, "connect") != NULL)     // 成功连上之后, 发个测试数据
		    ret = webSocket_send(fd, (unsigned char *)"Hello !", strlen("Hello !"), false, WCT_TXTDATA);
		else if(strstr(buf, "Hello") != NULL)
		    ret = webSocket_send(fd, (unsigned char *)"I am Server_Test", strlen("I am Server_Test"), false, WCT_TXTDATA);
		else
		    ret = webSocket_send(fd, (unsigned char *)"You are carefree ...", strlen("You are carefree ..."), false, WCT_TXTDATA);
		// ... ...
		// ...
	}
	return ret;
...
int main(void)
{
    ...
    //===== 开辟线程, 管理服务器 =====
    ret = pthread_create(&sever_thread_id, NULL, (void *(*)(void*))&server_thread_fun, (void *)(&ws));  // 传递参数到线程
	if(ret != 0)
	{
	...
	while(1)
	{
	    for(i = 0; i < EPOLL_RESPOND_NUM; i++)
	    {
	        if(ws.client_fd_array[i][1] != 0 && ws.client_fd_array[i][0] > 0)
	        {
	            ...
	            ret = webSocket_send(client_fd,(unsigned char *) "\\O^O/  <-.<-  TAT  =.=#  -.-! ...", strlen("\\O^O/  <-.<-  TAT  =.=#  -.-! ..."), false, WCT_TXTDATA);
